...
def get_dashboard_stats(db: Session = Depends(get_db)):
    ...
    in_progress_ros = db.query(func.count(RepairOrder.ro_id)).filter(
    ...
    # Last 7 days revenue from quotes
    revenue_7d = db.query(func.sum(Quote.total_amount)).filter(
        Quote.created_at >= seven_days_ago
    ).scalar() or 0.0

    quotes_7d = db.query(func.count(Quote.quote_id)).filter(
        Quote.created_at >= seven_days_ago
    ).scalar() or 0
    ...
    return DashboardStats(
        total_ros        = total_ros,
        open_ros         = open_ros,
        in_progress_ros  = in_progress_ros,
